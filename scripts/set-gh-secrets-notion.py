#!/usr/bin/env python3
"""Set multiple GitHub Actions secrets for the notion-poll workflow."""
import sys
import json
...
SECRETS = {
    ...
    "NOTION_DATABASE_ID": "REDACTED_DB_ID",
}
...
for name, value in SECRETS.items():
    ...
    status, resp = api("PUT", f"actions/secrets/{name}", {
        "encrypted_value": encrypted_b64,
        "key_id": key_id,
    })
    if status in (201, 204):
        print(f"✅ Secret '{name}' set successfully (HTTP {status})")
    else:
